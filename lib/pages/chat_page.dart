...
class _ChatPageState extends State<ChatPage> {
  // Класс состояния для ChatPage
  final TextEditingController _messageController =
      TextEditingController(); // Контроллер для управления текстовым полем
  final ChatService _chatService =
      ChatService(); // Экземпляр сервиса чата для отправки и получения сообщений
  final FirebaseAuth _firebaseAuth =
      FirebaseAuth.instance; // Экземпляр для работы с аутентификацией Firebase
...
// Пользователь выбирает изображение из галереи
    final XFile? image = await _picker.pickImage(source: ImageSource.gallery);
// Если изображение не выбрано, выходим из функции
    if (image == null) return;
// Преобразовываем путь в файл
    File file = File(image.path);
// Создаем уникальное имя файла на основе времени
    String fileName =
        'images/${DateTime.now().millisecondsSinceEpoch}_${image.name}';
    FirebaseStorage storage = FirebaseStorage.instance; // Экземпляр хранилища
